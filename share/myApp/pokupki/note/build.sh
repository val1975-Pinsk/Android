...
javac -deprecation -cp $PLATFORM/android.jar -d build/obj build/gen/pokuki/note/R.java java/pokupki/note/*.java

echo "Запуск d8..."
echo ""
$BUILD_TOOLS/d8 --release --lib $PLATFORM/android.jar --output build/apk/ build/obj/pokuki/note/*.class

echo "Упаковка в apk..."
echo ""
$BUILD_TOOLS/aapt package -f -M AndroidManifest.xml -S ./res -I $PLATFORM/android.jar -F build/pokupkiNote.unsigned.apk build/apk/

$BUILD_TOOLS/zipalign -f -p 4 build/pokupkiNote.unsigned.apk build/pokupkiNote.aligned.apk
...
$BUILD_TOOLS/apksigner sign --ks keystore.jks --ks-key-alias androidkey --ks-pass pass:android --out build/pokupkiNote.apk build/pokupkiNote.aligned.apk
